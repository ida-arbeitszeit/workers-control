...
@dataclass
class AcceptCollaborationRequestPresenter:
    translator: Translator
    notifier: Notifier
    url_index: UrlIndex

    def render_response(self, response: AcceptCollaborationResponse) -> ViewModel:
        if not response.is_rejected:
            self.notifier.display_info(
                self.translator.gettext("Collaboration request has been accepted.")
            )
        else:
            ...
            match response.rejection_reason:
                ...
                case (
                    ...
                    | rejection_reason.plan_has_collaboration
