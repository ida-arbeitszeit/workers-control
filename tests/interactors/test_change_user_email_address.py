...
class ChangeUserEmailAddressTests(BaseTestCase):
    def setUp(self) -> None:
        super().setUp()
        self.interactor = self.injector.get(ChangeUserEmailAddressInteractor)
        self.log_in_member_interactor = self.injector.get(
            log_in_member.LogInMemberInteractor
        )
    ...
    def test_that_member_can_log_in_with_new_email_after_address_change(
        ...
        request = Request(user=member, new_email=new_email)
        self.interactor.change_user_email_address(request)
    ...
    def can_log_in_member(self, email_address: str, password: str) -> bool:
        response = self.log_in_member_interactor.log_in_member(
            request=log_in_member.LogInMemberInteractor.Request(
    ...
    def cannot_confirm_member_email_address(self, email: str) -> None:
        confirm_member_interactor = self.injector.get(
            confirm_member.ConfirmMemberInteractor
        )
        confirm_member_request = confirm_member.ConfirmMemberInteractor.Request(
            email_address=email
        )
        response = confirm_member_interactor.confirm_member(confirm_member_request)
