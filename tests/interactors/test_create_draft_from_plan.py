...
class CreateDraftFromPlanTests(BaseTestCase):
    def setUp(self) -> None:
        super().setUp()
        self.interactor = self.injector.get(interactor.CreateDraftFromPlanInteractor)
        self.my_plans_interactor = self.injector.get(
            show_my_plans.ShowMyPlansInteractor
        )
        self.get_draft_details_interactor = self.injector.get(
    ...
    def test_creating_draft_from_random_uuids_produces_rejection_response(self) -> None:
        response = self.interactor.create_draft_from_plan(
    ...
    def test_that_company_has_an_additional_draft_after_succesful_request(
        ...
        self.interactor.create_draft_from_plan(
            request=interactor.Request(
                plan=plan,
                company=company,
            )
    ...
    def count_drafts_for_company(self, company: UUID) -> int:
        ...
        response = self.my_plans_interactor.show_company_plans(request)
        return len(response.drafts)

    def get_draft_details(self, draft: UUID) -> get_draft_details.DraftDetailsSuccess:
        response = self.get_draft_details_interactor.execute(draft)
        assert response
        return response
