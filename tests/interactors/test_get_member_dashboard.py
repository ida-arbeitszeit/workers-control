...
class InteractorTests(BaseTestCase):
    def setUp(self) -> None:
        super().setUp()
        self.interactor = self.injector.get(GetMemberDashboardInteractor)
        self.invite_worker_to_company = self.injector.get(
            InviteWorkerToCompanyInteractor
        )
    ...
    def test_show_correct_invite_id(self):
        ...
        request = get_member_dashboard.Request(member=self.member)
        get_dashboard_response = self.interactor.get_member_dashboard(request)
