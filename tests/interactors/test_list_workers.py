...
def test_list_workers_response_is_empty_for_nonexisting_company(
    interactor: list_workers.ListWorkersInteractor,
) -> None:
    response: list_workers.Response = interactor.execute(make_request(company=uuid4()))
...
def test_list_workers_response_is_empty_for_company_without_worker(
    ...
    response = interactor.execute(make_request(company=company))
    assert not response.workers
...
def test_list_workers_response_includes_multiple_company_workers(
    ...
    response: list_workers.Response = interactor.execute(
        make_request(company=company.id)
    )
