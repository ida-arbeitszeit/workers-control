...
class RegisterMemberTests(BaseTestCase):
    ...
    def test_that_token_was_delivered_to_registering_user(self) -> None:
        ...
        self.interactor.register_member(
            RegisterMemberInteractor.Request(
                email=expected_email_address, **request_args
            )
        )
        self.assertEqual(
    ...
    def test_confirmation_is_required_if_email_is_not_already_known(self) -> None:
        request_args = DEFAULT.copy()
        response = self.interactor.register_member(
            RegisterMemberInteractor.Request(**request_args)
        )
    ...
    def test_that_uuid_returned_is_the_same_as_when_logging_in(self) -> None:
        register_response = self.interactor.register_member(
            RegisterMemberInteractor.Request(**DEFAULT)
        )
        login_response = self.login_interactor.log_in_member(
    ...
    def test_registering_a_member_with_same_email_as_company_does_not_require_company_to_reconfirm_email(
        ...
        response = self.confirm_company_interactor.confirm_company(
            ConfirmCompanyInteractor.Request(email_address=expected_email)
        )
