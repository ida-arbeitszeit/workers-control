...
class InteractorTests(BaseTestCase):
    def setUp(self) -> None:
        super().setUp()
        self.interactor = self.injector.get(RejectPlanInteractor)
    ...
    def test_that_an_unreviewed_plan_will_be_rejected(self) -> None:
        ...
        rejection_response = self.interactor.reject_plan(self.create_request(plan=plan))
    ...
    def test_cannot_reject_plan_that_was_already_rejected(self) -> None:
        ...
        self.interactor.reject_plan(request)
        rejection_response = self.interactor.reject_plan(request)
    ...
    def test_that_rejection_date_is_set_correctly(self) -> None:
        ...
        response = self.interactor.reject_plan(self.create_request(plan=plan))
    ...
    def test_that_planning_company_cannot_register_consumption_for_rejected_plan(
        ...
        self.interactor.reject_plan(self.create_request(plan=plan))
    ...
    def test_that_company_account_balance_is_not_changed_when_plan_is_rejected(
        ...
        self.interactor.reject_plan(self.create_request(plan=rejected_plan))
        self.assertEqual(
    ...
    def create_request(self, plan: UUID) -> RejectPlanInteractor.Request:
        return RejectPlanInteractor.Request(plan=plan)
