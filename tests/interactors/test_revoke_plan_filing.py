...
class PlanGetsRevokedTests(BaseTestCase):
    def setUp(self) -> None:
        super().setUp()
        self.interactor = self.injector.get(RevokePlanFilingInteractor)
        self.show_my_plans = self.injector.get(ShowMyPlansInteractor)

    def test_interactor_is_unsuccessful_if_requesting_company_does_not_exist(
        self,
    ) -> None:
        response = self.interactor.revoke_plan_filing(
            self.create_request(requester=uuid4())
        )
        assert response.is_rejected
        assert response.rejection_reason == rejection_reason.requester_not_found

    def test_interactor_is_unsuccessful_if_plan_does_not_exist(self) -> None:
        response = self.interactor.revoke_plan_filing(self.create_request(plan=uuid4()))
        assert response.is_rejected
        assert response.rejection_reason == rejection_reason.plan_not_found

    def test_interactor_is_unsuccessful_if_requester_is_not_planner_of_plan(
        self,
    ) -> None:
        response = self.interactor.revoke_plan_filing(self.create_request())
        assert response.is_rejected
        assert response.rejection_reason == rejection_reason.requester_is_not_planner

    def test_interactor_is_unsuccessful_if_plan_is_approved_and_not_expired(
        self,
    ) -> None:
    ...
    def test_interactor_is_unsuccessful_if_plan_is_expired(self) -> None:
    ...
    def test_no_draft_id_is_in_response_if_interactor_is_unsuccessful(
        self,
    ) -> None:
        response = self.interactor.revoke_plan_filing(
            self.create_request(requester=uuid4())
    ...
    def test_after_revoking_successfully_the_plan_does_not_show_in_companys_plans_anymore(
        ...
        self.interactor.revoke_plan_filing(
            self.create_request(plan=plan, requester=planner)
        )
    ...
    ) -> RevokePlanFilingInteractor.Request:
        if requester is None:
            requester = self.company_generator.create_company()
        if plan is None:
            plan = self.plan_generator.create_plan()
        return RevokePlanFilingInteractor.Request(requester=requester, plan=plan)

    def plan_is_a_plan_of_company(self, company: UUID, plan: UUID) -> bool:
        interactor_request = ShowMyPlansRequest(company_id=company)
        response = self.show_my_plans.show_company_plans(request=interactor_request)
        company_plans = [
            p.id
            for p in (
