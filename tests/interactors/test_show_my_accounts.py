...
class ShowCompanyAccountsTest(BaseTestCase):
    def setUp(self) -> None:
        super().setUp()
        self.interactor = self.injector.get(ShowCompanyAccountsInteractor)
    ...
    def test_that_balance_of_product_account_reflects_private_consumption_that_took_place(
        ...
        balance_before_consumption = self.interactor.execute(
            request=ShowCompanyAccountsRequest(company=company)
        ).balances[3]
        ...
        balance_after_consumption = self.interactor.execute(
            request=ShowCompanyAccountsRequest(company=company)
        ).balances[3]
