...
class InteractorTestBase(BaseTestCase):
    def setUp(self) -> None:
        ...
        self.interactor = self.injector.get(
            show_p_account_details.ShowPAccountDetailsInteractor
        )
        self.company = self.company_generator.create_company_record()
        self.means_account = self.company.means_account

    def create_interactor_request(
        self, company_id: UUID
    ) -> show_p_account_details.ShowPAccountDetailsInteractor.Request:
        return show_p_account_details.ShowPAccountDetailsInteractor.Request(
            company=company_id
        )


class ShowMeansAccountTests(InteractorTestBase):
...
class MeansAccountBalanceTests(InteractorTestBase):
    ...
    def test_balance_is_zero_when_transfer_took_place_to_other_account_of_company(
        ...
        self.transfer_generator.create_transfer(debit_account=self.company.work_account)
        self.transfer_generator.create_transfer(
...
class MeansAccountTransferTests(InteractorTestBase):
    ...
    def test_that_transfer_is_returned_when_means_account_of_company_is_debit_account(
        self,
    ) -> None:
        self.transfer_generator.create_transfer(debit_account=self.means_account)
        response = self.interactor.show_details(
    ...
    def test_that_transfer_is_returned_when_means_account_of_company_is_credit_account(
        self,
    ) -> None:
        self.transfer_generator.create_transfer(credit_account=self.means_account)
        response = self.interactor.show_details(
            self.create_interactor_request(self.company.id)
        )
        assert response.transfers
    ...
    def test_that_transfers_to_other_accounts_of_company_do_not_show_up(self) -> None:
        ...
        self.transfer_generator.create_transfer(
            credit_account=self.company.work_account
        )
        self.transfer_generator.create_transfer(
    ...
    def test_two_transfers_are_recorded_in_descending_order(self) -> None:
        ...
        self.transfer_generator.create_transfer(
            ...
            credit_account=self.means_account,
        )
        self.datetime_service.advance_time(timedelta(days=1))
        self.transfer_generator.create_transfer(
            type=TransferType.credit_a, credit_account=self.means_account
        )
        response = self.interactor.show_details(
            self.create_interactor_request(self.company.id)
        )
...
class MeansAccountPlottingTests(InteractorTestBase):
