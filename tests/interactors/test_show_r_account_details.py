...
class InteractorTestBase(BaseTestCase):
    def setUp(self) -> None:
        ...
        self.raw_material_account = self.company.raw_material_account
...
class RawMaterialAccountBalanceTests(InteractorTestBase):
...
class RawMaterialAccountTransferTests(InteractorTestBase):
    ...
    def test_that_transfer_is_returned_when_raw_material_account_of_company_is_debit_account(
        self,
    ) -> None:
        self.transfer_generator.create_transfer(debit_account=self.raw_material_account)
        response = self.interactor.show_details(
    ...
    def test_that_transfer_is_returned_when_raw_material_account_of_company_is_credit_account(
        self,
    ) -> None:
        self.transfer_generator.create_transfer(
            credit_account=self.raw_material_account
        )
        response = self.interactor.show_details(
            self.create_interactor_request(self.company.id)
        )
    ...
    def test_two_transfers_are_recorded_in_descending_order(self) -> None:
        ...
        self.transfer_generator.create_transfer(
            type=TransferType.credit_a, credit_account=self.raw_material_account
        )
        response = self.interactor.show_details(
            self.create_interactor_request(self.company.id)
        )
