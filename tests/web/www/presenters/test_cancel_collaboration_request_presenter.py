...
class CancelCollaborationRequestPresenterTests(BaseTestCase):
    def setUp(self) -> None:
        super().setUp()
        self.presenter = self.injector.get(CancelCollaborationRequestPresenter)
    ...
    def test_successfull_cancel_request_response_is_presented_correctly(self) -> None:
        ...
        assert self.notifier.infos[0] == self.translator.gettext(
            "Collaboration request has been canceled."
        )
    ...
    def test_that_user_gets_redirected_to_my_collaborations_view(
        ...
        assert view_model.redirection_url == self.url_index.get_my_collaborations_url()
