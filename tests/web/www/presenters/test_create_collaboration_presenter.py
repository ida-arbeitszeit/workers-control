...
class CreateCollaborationPresenterTests(BaseTestCase):
    def setUp(self) -> None:
        super().setUp()
        self.presenter = self.injector.get(CreateCollaborationPresenter)
    ...
    def test_notification_returned_when_creation_was_rejected_because_collab_name_existed(
        self,
    ) -> None:
        self.presenter.present(REJECTED_RESPONSE_NAME_EXISTS)
        self.assertTrue(self._get_warning_notifications())

    def test_correct_notification_is_returned_when_creation_was_rejected_because_collab_name_existed(
        self,
    ) -> None:
        self.presenter.present(REJECTED_RESPONSE_NAME_EXISTS)
        self.assertIn(
...
class NavbarItemsTests(BaseTestCase):
    ...
    def test_navbar_links_to_my_collaborations_then_shows_create_as_current_page(
        self,
    ) -> None:
        items = self.presenter.create_navbar_items()
        self.assertEqual(len(items), 2)
        self.assertEqual(items[0].text, self.translator.gettext("My collaborations"))
        self.assertEqual(items[0].url, self.url_index.get_my_collaborations_url())
        self.assertEqual(items[1].text, self.translator.gettext("Create Collaboration"))
