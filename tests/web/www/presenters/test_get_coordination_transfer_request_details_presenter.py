...
class GetDetailsPresenterTests(BaseTestCase):
    ...
    def test_that_request_date_is_formatted_correctly_in_view_model(self) -> None:
    ...
    def test_that_candidate_name_is_displayed_in_view_model(self) -> None:
    ...
    def test_that_current_user_is_candidate_is_true_if_current_user_is_candidate(
        self,
    ) -> None:
        candidate = uuid4()
    ...
    def test_that_current_user_is_candidate_is_false_if_another_user_is_candidate(
        self,
    ) -> None:
        user = uuid4()
        candidate = uuid4()
    ...
    def test_that_request_is_pending_is_true_if_request_is_pending(self) -> None:
        response = self.get_interactor_response(request_is_pending=True)
        view_model = self.presenter.present(response)
        self.assertTrue(view_model.request_is_pending)

    def test_that_request_is_pending_is_false_if_request_is_not_pending(self) -> None:
        response = self.get_interactor_response(request_is_pending=False)
        view_model = self.presenter.present(response)
        self.assertFalse(view_model.request_is_pending)

    def test_that_request_status_reads_pending_if_request_is_pending(self) -> None:
        response = self.get_interactor_response(request_is_pending=True)
        view_model = self.presenter.present(response)
        self.assertEqual(view_model.request_status, self.translator.gettext("Pending"))

    def test_that_request_status_reads_closed_if_request_is_not_pending(self) -> None:
        response = self.get_interactor_response(request_is_pending=False)
        view_model = self.presenter.present(response)
