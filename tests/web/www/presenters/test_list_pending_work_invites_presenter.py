...
class ListPendingWorkInvitesPresenterTests(BaseTestCase):
    ...
    def test_no_invites_in_view_model_when_there_are_no_pending_invites(self) -> None:
        interactor_response = self.create_interactor_response([])
    ...
    def test_view_model_invite_has_member_id_from_interactor_response_as_string(
        self,
    ) -> None:
        member_id = uuid4()
        interactor_response = self.create_interactor_response(
    ...
    def test_view_model_invite_has_member_name_from_interactor_response(
    ...
    def create_interactor_response(
        ...
        return interactor.Response(
            pending_invites=[
                interactor.PendingInvite(member_id, member_name)
