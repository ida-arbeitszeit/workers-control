...
from workers_control.core.interactors import list_transfers
from workers_control.core.transfers import TransferType
from workers_control.web.pagination import DEFAULT_PAGE_SIZE
...
class PaginationTests(ListTransfersPresenterBase):
    ...
    def test_pagination_is_visible_if_total_results_is_greater_than_default_page_size(
        ...
        uc_response = self.create_interactor_response(total_results=total_results)
...
class ShosResultsTests(ListTransfersPresenterBase):
    def test_show_results_is_false_if_no_transfers(self) -> None:
        uc_response = self.create_interactor_response(transfers=[])
        view_model = self.presenter.present(uc_response)
...
class TotalResultsTests(ListTransfersPresenterBase):
    ...
    def test_total_results_is_same_as_in_interactor_response(
...
class ResultsTests(ListTransfersPresenterBase):
    ...
    def test_that_length_of_results_is_same_as_in_interactor_response(
    ...
    def test_that_transfer_date_is_converted_to_correct_format(self) -> None:
        ...
        assert view_model.results.rows[
            0
        ].date == self.datetime_formatter.format_datetime(date, fmt="%d.%m.%Y %H:%M")
