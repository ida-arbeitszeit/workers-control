...
class ReviewRegisteredConsumptionsPresenterTests(BaseTestCase):
    ...
    def test_that_an_interactor_response_without_consumptions_results_in_an_empty_view_model(
        self,
    ) -> None:
        interactor_response = Interactor.Response(consumptions=[])
        view_model = self.presenter.present(interactor_response)
        assert not view_model.consumptions

    def test_that_an_interactor_response_with_one_consumption_results_in_a_view_model_with_one_consumption(
        self,
    ) -> None:
        consumption = self._create_consumption()
        interactor_response = Interactor.Response(consumptions=[consumption])
        view_model = self.presenter.present(interactor_response)
        assert len(view_model.consumptions) == 1

    def test_that_an_interactor_response_with_two_consumptions_results_in_a_view_model_with_two_consumptions(
        self,
    ) -> None:
        consumptions = [self._create_consumption(), self._create_consumption()]
        interactor_response = Interactor.Response(consumptions=consumptions)
        view_model = self.presenter.present(interactor_response)
        assert len(view_model.consumptions) == 2

    def test_if_there_is_a_interactor_response_with_two_consumptions_the_view_model_shows_correct_product_names(
        self,
    ) -> None:
        consumptions = [
    ...
    def test_that_an_interactor_response_results_in_a_view_model_with_a_formatted_consumption_date(
        self,
    ) -> None:
    ...
    def test_that_an_interactor_response_results_in_a_view_model_with_the_consumer_name(
        self,
    ) -> None:
    ...
    def test_that_an_interactor_response_results_in_a_view_model_with_the_product_name(
        self,
    ) -> None:
    ...
    def test_that_an_interactor_response_results_in_a_view_model_with_a_plan_url(
        self,
    ) -> None:
        consumption = self._create_consumption()
        interactor_response = Interactor.Response(consumptions=[consumption])
